...
class AsyncUnitTest : public Test
{
public:
...
using AsyncPriorityUnitTestData = struct {
	int priorities[5];
	int sequence[5];
};

static const AsyncPriorityUnitTestData AsyncPriorityUnitTestDataList[] = {
...
class AsyncPriorityUnitTest
	: public AsyncUnitTest
	, public WithParamInterface<AsyncPriorityUnitTestData>
{
public:
	void SetUp() {
		AsyncUnitTest::SetUp();
	...
	void TearDown() {
		AsyncUnitTest::TearDown();
	}
};

TEST_P(AsyncPriorityUnitTest, sequence)
{
	auto& param = GetParam();
...
INSTANTIATE_TEST_SUITE_P(priority, AsyncPriorityUnitTest, ValuesIn(AsyncPriorityUnitTestDataList));
